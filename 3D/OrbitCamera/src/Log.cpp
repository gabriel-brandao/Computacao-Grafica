...
const wchar_t* LOG_FILE = L"log.txt";
...
{
    // get dir where the application is located
    int bufferSize = 1024;
    ...
    std::wstring path = L"";        // dir alone without the filename
    std::size_t index = filePath.find_last_of(L"/\\");
    if(index != std::string::npos)
        path = filePath.substr(0, index+1);
    delete [] buffer;   // dealloc tmp buffer

    // open log file
    logFile.open(path + LOG_FILE, std::ios::out);
    if(logFile.fail())
        return;
